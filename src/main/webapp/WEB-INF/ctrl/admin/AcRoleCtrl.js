angular.module('myApp').controller('AcRoleCtrl', function($scope, $rootScope, $mdDialog, $http,$anchorScroll, $location){
	...
	$scope.search = function() {
		...
		$http.post(ctx + '/admin/AcRoleCtrl/search', $scope.searchForm).success(function(data){
			$scope.dataList = data;
			$scope.isLoading = false;
		});
	};
	...
	$scope.edit = function() {
		...
		$mdDialog.show({
			templateUrl: ctx + '/templates/admin/AcRoleEditTpl.html?v=',
			controller: 'AcRoleEditCtrl',
	...
	$scope.del = function() {
		...
		$mdDialog.show(confirm).then(function() {
			$scope.promise = $http.get(ctx + '/admin/AcRoleCtrl/delete?id='+item.id).success(function(data){
...
myApp.controller('AcRoleEditCtrl', function($scope,$mdDialog,$http,id){
	$scope.editForm = {};
	$scope.editForm.id = id;
	if(id && id > 0) {
		$scope.title = '编辑';
		$scope.promise = $http.get(ctx + '/admin/AcRoleCtrl/getDetailInfo?id='+id).success(function(data){
	...
	$scope.save = function() {
		$scope.promise = $http.post(ctx + '/admin/AcRoleCtrl/save', $scope.editForm).success(function(data){
			if(data.result == 'success') {
