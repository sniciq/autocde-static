(function (angular, undefined) {
	var module = angular.module('ngTreeView', []);
		...
		return {
			...
			link: function (scope, element, attrs, controller) {
				...
				scope.getFolderIconClass = function () {
					//return 'glyphicon ' + (scope.expanded && scope.hasChildren() ? 'glyphicon-folder-open' : '');
				...
				scope.selectNode = function (event) {
					...
					if(scope.hasChildren()) {
						if(scope.expanded) {
							scope.node.icon = 'folder';	
						}
						else {
							scope.node.icon = 'folder_open';
						}
					}
					else {
					}
					...
					var template =
						'<div class="tree-folder" ng-repeat="node in ' + attrs.treeViewNode + '.' + foldersProperty + '">' +
							'<a href="#" class="tree-folder-header inline" ng-click="selectNode($event)" ng-class="{ selected: isSelected(node) }">' +
								'<i style="font-size: 18px;" ng-if="node.' + foldersProperty + '.length > 0" class="material-icons">{{node.icon}}</i> ' +
								'<span class="tree-folder-name" ng-if="node.' + foldersProperty + '.length > 0">{{ node.' + displayProperty + ' }}</span> ' +
								
								'<span class="tree-item-name" ng-if="node.' + foldersProperty + '.length == 0"><i style="font-size: 18px;" class="material-icons">{{node.icon}}</i> {{ node.' + displayProperty + ' }}</span> ' +
							'</a>' +
						...
						'<a href="#" class="tree-item" ng-repeat="file in ' + attrs.treeViewNode + '.' + filesProperty + '" ng-click="selectFile(file, $event)" ng-class="{ selected: isSelected(file) }">' +
							'<span class="tree-item-name"><i style="font-size: 18px;" class="material-icons">{{node.icon}}</i> {{ file.' + displayProperty + ' }}</span>' +
						'</a>';
